import pygame, random, sys, time # imports all the necessary libraries 
...
def play(): # creates the difficulty selection screeen
    pygame.display.set_caption("Bird Hunt - Difficulty")
    running = True

    while running:
        menutext = menu_font.render("Difficulty", True, ("White"))
        menurect = menutext.get_rect(center = (270, 50))

        Bg = pygame.image.load("Assets/Background.png")
        Bgrect = Bg.get_rect(center = (270, 240))

        easybg = pygame.image.load("Assets/Button Rect.png")
        easyrect = easybg.get_rect(center = (270, 200))
        easytext = menu_font.render("Easy", True, (0, 0, 0))
        easytextrect = easytext.get_rect(center = (270, 200))

        mediumbg = pygame.image.load("Assets/Button Rect.png")
        mediumrect = mediumbg.get_rect(center = (270, 350))
        mediumtext = menu_font.render("Medium", True, (0, 0, 0))
        mediumtextrect = mediumtext.get_rect(center = (270, 350))
        ...
        screen.blit(Bg, Bgrect)
        screen.blit(menutext, menurect)
        screen.blit(mediumbg, mediumrect)
        screen.blit(mediumtext, mediumtextrect)
        screen.blit(easybg, easyrect)
        screen.blit(easytext, easytextrect)
        pygame.display.flip() # puts the current frame on the screen
        clock.tick(60)

def easygame(): # creates the main game screen
    beginning = time.time()
    ...
    while running:
        ...
        points_surface = my_font.render(str(kills), False, (0, 0, 0)) # writes the pontuation down on screen

        time_surface = my_font.render(str(countertime), False, (0, 0, 0)) # writes the pontuation down on screen
...
def mediumgame(): # creates the main game screen
    beginning = time.time()
    pygame.display.set_caption("Bird Hunt - Play")
    running = True
    x = 0
